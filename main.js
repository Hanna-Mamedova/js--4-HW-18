...
function wordsList(str, subStr) {
    let regExp = new RegExp ('\\.|,|!|\\?|;|:|"', 'gui');
    str = str.replace(regExp, ' ').toLowerCase().split(" ");
    str = [...new Set(str)];
    res = str.filter((item) => item.indexOf(subStr) > -1);
...
// Сделать 2х-значным месяц и минуты! +++
// Убрать секунды!! 
...
function getDateAgo(date, days) {
    ...
    let res = date.replace(/, \d{2}:\d{2}:\d{2}/, '');
...
let Car = function (engine, model, name, year) {
    ...
    this.info = function() {
        var res = this.name + ` ` + this.model + `, ` + this.engine + `cc, year ` + this.year + `, ` + this.used;
        return res;
    };
}
